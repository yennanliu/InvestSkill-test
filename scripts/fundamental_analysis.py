...
=======================
1. Reads the prompt from InvestSkill/prompts/fundamental-analysis.md
2. Fetches live + historical financial data via yfinance
3. Calls the OpenAI API and saves the report as Markdown (Traditional Chinese)

Usage
-----
  python scripts/fundamental_analysis.py AAPL
  python scripts/fundamental_analysis.py TSLA --model gpt-4o-mini --max-tokens 8000
  python scripts/fundamental_analysis.py AAPL --prompt-file /path/to/fundamental-analysis.md
...
DEFAULT_PROMPT_FILE = Path("InvestSkill/prompts/fundamental-analysis.md")
...
def fetch_stock_data(ticker: str) -> str:
    ...
    # --- Income Statement (key TTM figures) ---
    lines += [
    ...
    lines += [
        ...
        f"- Revenue/Employee: {_fmt(info.get('revenuePerShare'), '$')}",
        "",
    ]
    ...
    # --- Historical financials (last 4 years from yfinance statements) ---
    try:
        fin = t.financials  # income statement, columns = fiscal year dates
...
def generate_report(ticker: str, prompt: str, model: str, max_tokens: int) -> str:
    ...
    print(f"Calling OpenAI ({model}) for {ticker.upper()} fundamental analysis...")
...
def save_report(ticker: str, content: str, output_dir: Path, model: str = "openai") -> Path:
    ...
    base = f"fundamental_analysis_{today}_{model}"
    ...
    frontmatter = (
        ...
        f"model: {model}\n"
        "language: zh-TW\n"
        f"generated_by: OpenAI API (scripts/fundamental_analysis.py)\n"
        "---\n\n"
    )
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Generate fundamental analysis report via InvestSkill + OpenAI")
