#!/usr/bin/env python3
"""
fundamental_analysis_gemini.py — thin wrapper.

Fundamental analysis report via the layered ``analysis`` package. All logic lives
in scripts/analysis/ (config · prompts · data · llm · pipeline · publish); this
entrypoint just selects the analysis type. Defaults to the Gemini provider but
accepts ``--provider openai|claude``.

Usage:
  python scripts/fundamental_analysis_gemini.py AAPL
  python scripts/fundamental_analysis_gemini.py NVDA --max-tokens 20000

Environment: GEMINI_API_KEY (or OPENAI_API_KEY / ANTHROPIC_API_KEY per --provider)
"""

from analysis.cli import run_single

if __name__ == "__main__":
    run_single("fundamental-analysis")
